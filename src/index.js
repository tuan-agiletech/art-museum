...
function setImageToMesh(mesh, imgUrl) {
  // Tính aspect ratio từ GEOMETRY thật (không phụ thuộc camera/viewport)
  const geometrySize = new THREE.Vector3();

  // Tính bounding box nếu chưa có
  ...
  // Aspect ratio thật của mesh (chiều rộng / chiều cao)
  // Với plane thường là geometrySize.x / geometrySize.y
  const meshAspect = geometrySize.x / geometrySize.y;

  console.log("🎯 Mesh geometry:", mesh.name, {
    size: `${geometrySize.x.toFixed(3)} x ${geometrySize.y.toFixed(
      3
    )} x ${geometrySize.z.toFixed(3)}`,
    aspect: meshAspect.toFixed(3),
  });

  const imageMaterial = createImageMaterial(imgUrl, meshAspect, mesh.geometry);
...
document.body.addEventListener("uploadevent", (event) => {
  ...
  annotationMesh[img_id].annotationDiv.setAnnotationDetails(
    title,
    description,
    name
  );
...
function loadModel() {
  ...
  loader.load(
    ...
    (xhr) => {
      ...
      const progress =
        xhr.total > 0 ? (xhr.loaded / xhr.total) * 100 : xhr.loaded / 60000; // a fixed number of bytes
