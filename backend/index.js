...
// Define API endpoints with /api prefix
app.get("/api", (req, res) => {
  res.json({
    success: true,
    message: "Art Museum API is running",
    endpoints: {
      list: "/api/list/:museum (GET)",
      upload: "/api/upload (POST)",
    },
  });
});

app.get("/api/list/:museum", async (req, res) => {
  ...
  try {
    const { data, error: checkError } = await supabase
      .from("museum")
      .select(
        "id, created_at, img_id, img_cid, museum, title, description, price, name"
      )
      .eq("museum", museum);
    ...
    return res
      .status(200)
      .json({ success: true, data, message: "Items retrieved successfully" });
  } catch (error) {
    return res
      .status(500)
      .json({ success: false, message: "List error", error: error.message });
  }
});
...
app.post("/api/upload", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    if (existingEntries.length > 0) {
      ...
      const { values, error } = await supabase.from("museum").insert({
        ip_address: ipAddress,
        img_id: img_id,
        title,
        description,
